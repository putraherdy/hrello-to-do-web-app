<template>
  ...
  >
    <div class="d-block">
      <v-container fluid class="hrello-topbar">
        <div class="d-flex justify-space-between">
          <v-icon @click="drawer = true">mdi-menu</v-icon>
          <nuxt-link to="/">
            <v-row no-gutters align="center" justify="space-between">
              <h3 class="logo">Hrello</h3>
            </v-row>
          </nuxt-link>
    ...
    <h1>{{ board.title }}</h1>
    <p>created {{ board.dateCreated | formatDate }}</p>
    <div class="d-flex flex-row pr-6 pt-3">
      ...
      >
        <div class="d-flex flex-row justify-space-between">
          <h4>{{ list.title }}</h4>
          <v-icon medium @click="deleteList(list.id)">mdi-delete-outline</v-icon>
        </div>
